...
void Manager_3DTools::onEvent( Event* p_event )
{
				...
				if(e->isPressed == true)
				{
					if(m_theSelectionTool)
					{
						...
						if(currentlyChosenTransformTool)
						{
							...
							m_theSelectionTool->beginSelection(xm_rayOrigin, xm_rayDir, camView, camProj, *m_viewPort, mouseCursorPoint, currentlyChosenTransformTool);
	...
	case EVENT_SET_TOOL:
		{
			Enum::ToolType tool = static_cast<Enum::ToolType>(SETTINGS()->selectedTool());
			switch(tool)
			{
