...
void GeometryGenerator::createLineListCircle(XMVECTOR &center, float &radius, int &points, XMVECTOR &color, MeshData2& meshData, char axis)
{
	unsigned int nrOfLinePointsToSet = points;
	float angleInBetweenPoints = ((2 * Math::Pi) / nrOfLinePointsToSet); // / (180 * MathHelper::Pi);

	XMMATRIX currentRotation;
	XMVECTOR currentVector;
	Vertex2 currentVertex;
	XMStoreFloat4(&currentVertex.Color, color);

	XMVECTOR rightVec = XMVectorSet(1.0f, 0.0f, 0.0f, 0.0f);
	XMVECTOR startPoint; // = center + rightVec;
	...
	switch (axis)
	{
		case 'x':
			...
			startPoint = center + XMVectorSet(0.0f, 0.0f, -radius, 1.0f);
			break;
		case 'y':
			rotAxis = XMVectorSet(0.0f, 1.0f, 0.0f, 1.0f);
			startPoint = center + XMVectorSet(0.0f, 0.0f, radius, 1.0f);
			break;
		case 'z':
			rotAxis = XMVectorSet(0.0f, 0.0f, 1.0f, 1.0f);
			startPoint = center + XMVectorSet(radius, 0.0f, 0.0f, 1.0f);
			break;
	}

	for(unsigned int i = 0; i < nrOfLinePointsToSet; ++i)
	{
		//meshData.Vertices.push_back(currentVertex); 

		currentRotation = XMMatrixRotationAxis(rotAxis, angleInBetweenPoints * i);

		currentVector = XMVector3Transform(startPoint, currentRotation);

		XMStoreFloat3(&currentVertex.Position, currentVector);
...
void GeometryGenerator::CreateSphere(float radius, UINT sliceCount, UINT stackCount, MeshData2& meshData)
{
	...
	Vertex2 topVertex(0.0f, +radius, 0.0f, 1.0f, 1.0f, 1.0f, 0.0f);
	Vertex2 bottomVertex(0.0f, -radius, 0.0f, 1.0f, 1.0f, 1.0f, 0.0f);
	...
	for(UINT i = 1; i <= stackCount-1; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			Vertex2 v;
			v.Color = XMFLOAT4(1.0f, 1.0f, 1.0f, 0.0f);
...
//}
//
void GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, UINT sliceCount, UINT stackCount, XMVECTOR &color, MeshData2& meshData, XMMATRIX &localSpaceTransform)
{
	meshData.Vertices.clear();
	meshData.Indices.clear();
	...
	for(UINT i = 0; i < ringCount; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			XMStoreFloat3(&vertex.Position, XMVector3Transform(XMLoadFloat3(&vertex.Position), localSpaceTransform));

			vertex.Color = XMFLOAT4(color.m128_f32[0], color.m128_f32[1], color.m128_f32[2], 1.0f);
	...
	BuildCylinderTopCap(bottomRadius, topRadius, height, sliceCount, stackCount, color, meshData, localSpaceTransform);
	BuildCylinderBottomCap(bottomRadius, topRadius, height, sliceCount, stackCount, color, meshData, localSpaceTransform);
}

void GeometryGenerator::BuildCylinderTopCap(float bottomRadius, float topRadius, float height, 
											UINT sliceCount, UINT stackCount, XMVECTOR &color, MeshData2& meshData, XMMATRIX &localSpaceTransform)
{
	...
	for(UINT i = 0; i <= sliceCount; ++i)
	{
		...
		XMVECTOR tempPos = XMVectorSet(x, y, z, 1.0f);
		XMFLOAT3 tempPos2;
		XMStoreFloat3(&tempPos2, XMVector3Transform(tempPos, localSpaceTransform));

		meshData.Vertices.push_back( Vertex2(tempPos2.x, tempPos2.y, tempPos2.z, color.m128_f32[0], color.m128_f32[1], color.m128_f32[2], 1.0f) );
	}

	// Cap center vertex.

	//meshData.Vertices.push_back( Vertex2(0.0f, y, 0.0f, color.m128_f32[0], color.m128_f32[1], color.m128_f32[2], 1.0f) );
	XMVECTOR tempPos = XMVectorSet(0.0f, y, 0.0f, 1.0f);
	XMFLOAT3 tempPos2;
	XMStoreFloat3(&tempPos2, XMVector3Transform(tempPos, localSpaceTransform));
											   ...
											   UINT sliceCount, UINT stackCount, XMVECTOR &color, MeshData2& meshData, XMMATRIX &localSpaceTransform)
{
	...
	{
		meshData.Indices.push_back(centerIndex);
		meshData.Indices.push_back(baseIndex + i); // UltimateCoffee.exe!std::allocator<GeometryGenerator::Vertex2>::deallocate(GeometryGenerator::Vertex2 * _Ptr, unsigned int __formal) Line 586	C++
		meshData.Indices.push_back(baseIndex + i+1); 

	}
}

void GeometryGenerator::createLine(XMVECTOR &endPointA, XMVECTOR &endPointB, unsigned int pointsInBetweenEndPoints, XMVECTOR &colorAtEndPointA, XMVECTOR &colorAtEndPointB, XMMATRIX &localSpaceTransform, MeshData2& meshData)
{
	float distanceBetweenEndPoints = XMVector4Length(endPointA - endPointB).m128_f32[0];
	float distanceBetweenInBetweenPoints = distanceBetweenEndPoints / pointsInBetweenEndPoints;
